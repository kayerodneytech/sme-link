...
export function PosTerminal({
  ...
  const changeDue =
    paymentMethod === "cash" && Number.isFinite(cashValue)
      ? calculateChange(total, cashValue)
      : null;
  const cashReady =
    paymentMethod !== "cash" ||
    (Number.isFinite(cashValue) && cashValue >= total && total > 0);
      ...
      current
        .map((line) =>
          line.id === id
            ? {
                ...line,
                quantity: Math.max(0, Math.min(line.stock, line.quantity + amount)),
              }
            : line,
        )
  ...
  function searchKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    const exact = products.find((product) =>
      [product.barcode, product.sku].some(
        (code) => code && code.toLowerCase() === query.trim().toLowerCase(),
      ),
    );
    if (exact) add(exact);
  ...
  async function checkout() {
    ...
    if (paymentMethod === "cash") {
      if (!Number.isFinite(cashValue) || cashValue < total) {
        setMessage("Enter cash received that covers the total.");
        return;
      }
    }
    ...
    try {
      ...
      const breakdown = splitVatInclusive(total, vatRegistered, vatRate);
      setReceipt({
  ...
  return (
    <main className="pos-shell">
      <header className="pos-header">
        <div>
          <BusinessMenu mode="pos" posEnabled />
          <div>
            <strong>{businessName}</strong>
            <small>Retail point of sale</small>
          </div>
        </div>
        <div className="pos-shortcuts">
          <Link className="pos-history-link" href="/sales">
            Sales history
          </Link>
          <span>
            <kbd>/</kbd> Search
      ...
      <div className="pos-workspace">
        <section className="pos-catalogue">
          <div className="pos-search">
            ...
            <input
              ...
              placeholder="Search name, category, SKU or scan barcode"
              ref={searchRef}
              value={query}
            />
            <Barcode size={21} />
          ...
          <div className="pos-products">
            ...
            {!shownProducts.length && (
              <div className="pos-empty">
                ...
                <p>Try the name, category, SKU or barcode.</p>
              </div>
            )}
          </div>
        </section>
        ...
        <aside className="pos-cart">
          ...
          <div className="pos-cart-lines">
            ...
            {!cart.length && (
              <div className="pos-empty">
                ...
                <p>Tap a product or scan its barcode.</p>
              </div>
            )}
          </div>
          <div className="pos-checkout">
            ...
            {paymentMethod === "cash" && (
              <div className="form-grid pos-cash-grid">
                <div className="field">
                ...
                <div className="field">
                  <label htmlFor="pos-change">Change</label>
                  <input
                    className="input"
                    id="pos-change"
                    readOnly
                    value={
                      changeDue === null || !Number.isFinite(changeDue)
                        ? ""
                        : formatMoney(Math.max(changeDue, 0), currency)
                    }
                  />
                </div>
              </div>
            ...
            <div className="pos-totals">
              <div>
                <span>Subtotal</span>
                <strong>{formatMoney(tax.net, currency)}</strong>
              </div>
              <div>
                <span>VAT ({tax.rate.toFixed(tax.rate % 1 ? 2 : 0)}%)</span>
                <strong>{formatMoney(tax.vat, currency)}</strong>
              </div>
              <div className="pos-total">
                <span>Total</span>
                <strong>{formatMoney(tax.gross, currency)}</strong>
              </div>
            </div>

            {message && <p className="form-message form-message-error">{message}</p>}
            <button
              className="button button-primary pos-pay"
              disabled={!cart.length || paying || !cashReady}
      ...
      {receipt && (
        <div className="dialog-backdrop receipt-backdrop">
          <section className="dialog receipt-dialog">
            <button
