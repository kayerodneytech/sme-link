...
export function SaleForm() {
  ...
  return (
    <form className="grid-main" onSubmit={(event) => event.preventDefault()}>
      <div style={{ display: "grid", gap: 16 }}>
        <section className="card card-pad">
          ...
          <div className="form-grid">
            <div className="field"><label htmlFor="customer">Customer</label><select className="select" id="customer"><option>Walk-in customer</option><option>Makanaka Store</option><option>Rudo Moyo</option><option>Tawanda Mini Mart</option></select></div>
            <div className="field"><label htmlFor="payment-method">Payment method</label><select className="select" id="payment-method"><option>Cash</option><option>EcoCash</option><option>Bank transfer</option><option>Card</option></select></div>
          </div>
        </section>
              ...
              return (
                <div className="card record-card" key={`${index}-${line.productId}`}>
                  <div className="form-grid" style={{ alignItems: "end" }}>
                    <div className="field"><label htmlFor={`product-${index}`}>Product</label><select className="select" id={`product-${index}`} onChange={(event) => updateLine(index, { productId: event.target.value })} value={line.productId}><option value="">Choose product</option>{products.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.stock} available</option>)}</select></div>
      ...
      <aside>
        <section className="card card-pad" style={{ position: "sticky", top: 96 }}>
          ...
          <button className="button button-primary" style={{ width: "100%" }} type="submit"><Check size={18} /> Complete sale</button>
